...
namespace hooked_methods {
    DEFINE_HOOKED_METHOD(PaintTraverse, void, vgui::IPanel *this_, vgui::VPANEL panel, bool force, bool allow_force) {
        ...
        if (isHackActive() && (health_panel || panel_scope || motd_panel || motd_panel_sd) && ((hacks::shared::catbot::catbotmode && hacks::shared::catbot::anti_motd && (panel == motd_panel || panel == motd_panel_sd))))
            call_default = false;

        ...
        esp_font_size.installChangeCallback(
            [](settings::VariableBase<int> &var, int after)
            {
                if (after > 0 && after < 100)
                {
                    ...
                    fonts::esp->unload();
                    fonts::esp.reset(new fonts::font(paths::getDataPath("/menu/Font.ttf"), after));
#else
        ...
        center_font_size.installChangeCallback(
            [](settings::VariableBase<int> &var, int after)
            {
                if (after > 0 && after < 100)
                {
                    ...
                    fonts::center_screen.reset(new fonts::font(paths::getDataPath("/menu/Font.ttf"), after));
#else
...
#if ENABLE_GLEZ_DRAWING
    glez::preInit();
    fonts::menu.reset(new fonts::font(paths::getDataPath("/menu/Font.ttf"), 10));
    fonts::esp.reset(new fonts::font(paths::getDataPath("/menu/Font.ttf"), 10));
    fonts::center_screen.reset(new fonts::font(paths::getDataPath("/menu/Font.ttf"), 12));
#elif ENABLE_ENGINE_DRAWING
    fonts::menu.reset(new fonts::font(paths::getDataPath("/menu/Font.ttf"), 10, true));
